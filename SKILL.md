...
description: >-
  ...
  "lavora sul progetto X", "apri una sessione su <progetto>", "vault duet", "duet sul progetto",
  "nuova sessione di lavoro su <progetto>", "fai X per il progetto Y" — even if they don't say
...
1. Load or create `~/.claude/vault-duet.config.json` (`projects_root` — ask the user for it on first run, no built-in default). Resolve the project the user named to a vault directory; if ambiguous or missing, list available vaults and ask.
